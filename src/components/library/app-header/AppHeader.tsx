...
import { appInfo } from '../../../environment';
import Button from 'devextreme-react/button';
import './AppHeader.scss';

export const AppHeader = ({ menuToggleEnabled, toggleMenu, className }: AppHeaderProps) => {
  const themeContext = useContext(ThemeContext);
  const navigate = useNavigate()

  const onThemeChange = useCallback(() => themeContext?.switchTheme(), [])
  ...
  return <header className={`header-component ${className}`}>
    <Toolbar className='header-toolbar'>
      ...
      <Item location='after' locateInMenu='never'>
        <div><Button icon={`${themeContext?.theme !== 'dark' ? 'moon' : 'sun'}`} onClick={onThemeChange} stylingMode='text'/></div>
      </Item>
      <Item location='after' locateInMenu='never'>
        <div><Button icon={"fullscreen"} onClick={toggleFullScreen} stylingMode='text'/></div>
      </Item>
    </Toolbar>
  </header>
}